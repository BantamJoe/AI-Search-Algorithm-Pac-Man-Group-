...
public class GhostMove : MonoBehaviour {
	...
	public void InitializeGhost()
	{
		...
		state = State.Wait;
        timeToEndWait = (Time.time-1) + waitLength + GUINav.initialDelay;
...
";
        ...
        if (st == State.Init)
        {
            using (StringReader reader = new StringReader(data))
            {
                ...
                {
                    if (line.Length == 0) break; // DOES IT WORK???? No it doesn't
	...
	void Init()
	{
		...
		if(waypoints.Count == 0)
		{
			...
			InitializeWaypoints(state);
			timeToEndScatter = scatterLength;
    ...
    //My attempt of having inky learn something

    void Learn(Collider2D wall)
    {
        if (transform.name == "inky")
        {
